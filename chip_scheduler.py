...
class ChipRateLimiter:
    """筹码接口频次限制器（滑动窗口，默认 5次/分钟）"""
    ...
    def wait(self):
        ...
        if len(self._timestamps) >= self.max_calls:
            ...
            if wait_time > 0:
                logger.warning(f"[频次限制] 筹码接口已达 {self.max_calls}次/{self.window_seconds:.0f}s 上限，等待 {wait_time:.1f} 秒...")
...
_chip_rate_limiter = ChipRateLimiter()
...
def run_chip_distribution_task(
    ...
    try:
        ...
        for i, stock_info in enumerate(stocks_to_process, 1):
            ...
            # 筹码接口频次限制（默认 5次/分钟），超限时自动等待
            _chip_rate_limiter.wait()
