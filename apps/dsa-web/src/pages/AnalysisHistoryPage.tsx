...
const getEastMoneyUrl = (code: string): string => {
  ...
  // 已含交易所前缀: SH600000, SZ000001, BJ430000, HK00700
  const prefixMatch = upper.match(/^(SH|SZ|BJ|HK)(\d+)$/);
  if (prefixMatch) {
    return `https://quote.eastmoney.com/${prefixMatch[1].toLowerCase()}${prefixMatch[2]}.html`;
  }

  // 含交易所后缀: 600519.SH, 000001.SZ, 430000.BJ, 00700.HK (SS 也是上交所)
  const suffixMatch = upper.match(/^(\d+)\.(SH|SZ|SS|BJ|HK)$/);
  if (suffixMatch) {
    const mapSuffixToPrefix: Record<string, string> = { SH: 'sh', SZ: 'sz', SS: 'sh', BJ: 'bj', HK: 'hk' };
    return `https://quote.eastmoney.com/${mapSuffixToPrefix[suffixMatch[2]]}${suffixMatch[1]}.html`;
  }

  // HK 前缀无点: HK00700
  if (upper.startsWith('HK')) {
    return `https://quote.eastmoney.com/hk${upper.slice(2)}.html`;
  }

  // 纯 A 股代码，按首字母判断交易所
  const prefix = upper.startsWith('6') ? 'sh' : upper.startsWith('4') || upper.startsWith('8') ? 'bj' : 'sz';
  return `https://quote.eastmoney.com/${prefix}${upper}.html`;
};
                        ...
                        >
                          ...
                          <span className="text-sm font-medium text-foreground break-words">
                            {item.stockName || item.stockCode}
                              ...
                              <a
                                  href={getEastMoneyUrl(item.stockCode)}
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-xs text-muted-text font-mono break-all hover:text-primary hover:underline transition-colors"
                              >
                            {item.stockCode}
                          </a>
                            </div>
