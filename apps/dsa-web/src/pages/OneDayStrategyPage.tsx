...
/**
 * 一日持股策略问股页面
 * 基于 HomePage 改造，专门针对一日持股策略进行分析
 */
const OneDayStrategyPage: React.FC = () => {
  const {
    error: analysisError,
    setLoading,
        ...
        <div className="flex items-center gap-2 w-full min-w-0 flex-1" style={{ maxWidth: 'min(100%, 1168px)' }}>
          ...
          <div className="flex-1 relative min-w-0">
            <input
              ...
              placeholder="输入股票代码进行一日持股策略分析，如 600519、00700、AAPL"
      ...
      <section className="md:col-start-4 md:row-start-2 flex-1 overflow-y-auto overflow-x-auto px-3 md:px-0 md:pl-1 min-w-0 min-h-0">
        ...
        {isLoadingReport ? (
            ...
            <div className="flex items-center justify-end mb-2">
              <button
                ...
                onClick={() => {
                  ...
                  navigate(`/chat?stock=${encodeURIComponent(code)}&name=${encodeURIComponent(name)}&recordId=${rid}&strategy=one_day_hold`);
                }}
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <h3 className="text-base font-medium text-white mb-1.5">一日持股策略分析</h3>
            <p className="text-xs text-muted max-w-xs">
              输入股票代码进行一日持股策略分析，或从左侧选择历史报告查看
            </p>
            <div className="mt-4 p-3 bg-white/5 border border-white/10 rounded-lg max-w-sm">
              <h4 className="text-sm font-medium text-white mb-1">什么是「一日持股策略」？</h4>
              <p className="text-xs text-secondary text-left">
                一日持股策略是针对短期交易的策略，重点关注当日买入卖出的时机、止损止盈点位、盘中波动分析等。
                适合短线交易者、日内交易者使用。
              </p>
            </div>
          </div>
...
export default OneDayStrategyPage;